...
export default function DirectMessage() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${getAllDmMessageUrl}/${params.dmId}`, requestConfig)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("Socket connected with dmid", params.dmId);
      // join the DM room so server can emit to this room only
      if (params.dmId) {
        socket.emit("join dm", { dmId: params.dmId });
      }
    });
    ...
    return () => {
      ...
      if (params.dmId) {
        socket.emit("leave dm", { dmId: params.dmId });
      }
      socket.disconnect();
    };
  }, [token]);
        ...
        >
          <div className="flex justify-between items-center px-8 w-full">
            <div className="flex gap-2 items-center">
              ...
              <CustomAvatar
                statusSize={{ width: "8px", height: "8px" }}
                showStatusSize={{ width: "12px", height: "12px" }}
                containerSize={{ width: "25px", height: "25px" }}
                avatarSize={{ width: "16px", height: "16px" }}
            ...
            <div className="flex gap-5 items-center">
              ...
              <UserRound
                onClick={() => setShowProfile((prv) => !prv)}
                style={{
            ...
            >
              <div className="px-4">
                ...
                <div className="flex items-center">
                  <div className="flex items-center">
                    <h1 className="flex items-center text-sm text-neutral-400">
                      {" "}
                      1 Mutual Server
                    </h1>
                          ...
                          <div>
                            <div className="flex gap-2 items-center">
                              ...
                              <div className="text-neutral-400 text-sm">
                                {" "}
                                <span>{msg.date}</span>, <span>{msg.time}</span>{" "}
                              </div>
                            </div>
                            {msg.contents.map((cont) => (
                              <li>{cont}</li>
                            ))}
                          </div>
                        </div>
