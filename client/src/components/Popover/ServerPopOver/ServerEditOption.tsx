...
import ServerInviteModal from "../../Utils/Modal/ServerInviteModal";

export default function ServerEditOption({
  setShowServerOption,
  setServer,
  server,
}) {
  return (
    <div className="text-neutral-200 flex flex-col gap-1">
      <ServerInviteModal server={server} setServer={setServer}>
        <div
          onClick={() => setShowServerOption(false)}
          className="server_option cursor-pointer server_option flex justify-between items-center py-2 px-3 rounded-md"
        >
      ...
      <CreateChannelModal server={server} setServer={setServer}>
        <div
          onClick={() => setShowServerOption(false)}
          className="cursor-pointer server_option flex justify-between items-center py-2 px-3 rounded-md"
        >
